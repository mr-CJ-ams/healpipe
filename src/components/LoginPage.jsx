...
import { ArrowDown, ArrowRight, CheckCircle2, Chrome, CircleAlert, Database, GitBranch, LoaderCircle, Moon, RotateCcw, ShieldCheck, Sun, Waypoints } from 'lucide-react'
import api, { SESSION_KEY } from '../api'

const GOOGLE_CLIENT_ID = import.meta.env.VITE_GOOGLE_CLIENT_ID

export default function LoginPage({ onAuthenticated, theme, onToggleTheme }) {
  ...
  if (!showAuth) {
    return (
      <main className="landing-page text-slate-100">
        <nav className="landing-nav"><div className="landing-brand"><span className="brand-icon"><ShieldCheck size={17} /></span><span>HealPipe</span><span className="landing-brand-sub">Webhook reliability infrastructure</span></div><div className="landing-nav-actions"><button className="theme-button landing-theme-button" onClick={onToggleTheme} aria-label="Toggle color theme" title="Toggle color theme">{theme === 'light' ? <Moon size={16} /> : <Sun size={16} />}</button><button className="landing-login-link" onClick={() => openAuth('login')}>Log in</button><button className="landing-nav-cta" onClick={() => openAuth('signup')}>Get started <ArrowRight size={15} /></button></div></nav>
  ...
  return (
    <main className="auth-page flex min-h-screen items-center justify-center bg-[#091312] px-5 py-12 text-slate-100">
      <section className="panel auth-card w-full max-w-md p-8 text-center sm:p-10" id="auth-panel">
        ...
        <div className="auth-header-row"><button className="auth-back-link" onClick={() => setShowAuth(false)}><ArrowDown size={14} className="rotate-90" />Back to overview</button><button className="theme-button" onClick={onToggleTheme} aria-label="Toggle color theme" title="Toggle color theme">{theme === 'light' ? <Moon size={16} /> : <Sun size={16} />}</button></div>
